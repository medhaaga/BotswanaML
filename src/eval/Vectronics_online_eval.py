...
# def all_online_summary_eval_from_model(model, feature_cols, metadata, device, sampling_frequency=16, window_length=None, window_duration=30.0, smoothening_window_length=1, smoothening_hop_length=1, plot=True, dir=None):
...
#         acc_data  = create_windowed_features(full_day_data, window_length=window_length)
#         acc_data['individual ID'] = individual
#         acc_data['UTC Date [yyyy-mm-dd]'] = date

#         print(f"{individual} has {len(acc_data)} acc windows on {date}.")
...
# def all_online_summary_eval_from_clusters(kmeans,
...
#                                           smoothening_window_length: int = 1, 
#                                           smoothening_hop_length: int = 1, 
#                                           plot: bool = True, dir: str = None):
...
#         X = torch.tensor(acc_data[feature_cols].values, dtype=torch.float32)
#         if transform is not None:
#             X = transform(X.to(device))
#             X = X.cpu().numpy()
        
#         X_cluster_labels = kmeans.predict(X)
